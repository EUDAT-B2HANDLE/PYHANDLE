...
class Searcher(object):
    ...
    def __init__(self, **args):
        ...
        pyhandle.util.log_instantiation(LOGGER, 'Searcher', args, ['password','reverselookup_password'])
    ...
    def __search_handle(self, **args):
        ...
        if len(args) == 0:
            ...
            isnone = pyhandle.util.return_keys_of_value_none(args)
            if len(isnone) > 0:
                LOGGER.debug('search_handle: These keys had value None: '+str(isnone))
                args = pyhandle.util.remove_value_none_from_dict(args)
                if len(args) == 0:
    ...
    def create_revlookup_query(self, *fulltext_searchterms, **keyvalue_searchterms):
        ...
        fulltext_searchterms = pyhandle.util.remove_value_none_from_list(fulltext_searchterms)
        ...
        keyvalue_searchterms = pyhandle.util.remove_value_none_from_dict(keyvalue_searchterms)
